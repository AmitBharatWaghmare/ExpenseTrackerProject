...
async function myFunc(event) {
    try {
      ...
      const response=await axios.post("http://127.0.0.1:5000/expense/addexpense",myObj, {headers: {Authorization: token}});
  ...
  // //shown on screen
    ...
    delBtn.onclick = async () => {
      const token = localStorage.getItem("token");
     const deletedItem = await axios.delete(`http://127.0.0.1:5000/expense/deletedata/${myObj._id}`,{headers:{Authorization: token}});
      try {
        ul.removeChild(li);
      } catch (err) {
        console.log(err);
  ...
  window.addEventListener("DOMContentLoaded",()=>{
   ...
   async  function fetchdata(){
      ...
      if(isadmin){
        ...
        const response= await axios.get(`http://127.0.0.1:5000/expense/getExpenses?page=1&limit=${limit}`, { headers: { Authorization: token}})
  ...
  async function showLedboard(){
    ...
    inputElement.onclick=async()=>
     {
      const token=localStorage.getItem(`token`);
      const userLeadboardArray=await axios.get(`http://127.0.0.1:5000/premium/showLeaderBoard`, { headers: { Authorization: token } });
      var leadboardelem=document.getElementById('leaderboard');
      leadboardelem.innerHTML +='Leader Board';
      console.log(userLeadboardArray.data);
  ...
  document.getElementById(`rzp-button1`).onclick = async function (e) {
    const token = localStorage.getItem("token");
   const response = await axios.get("http://127.0.0.1:5000/purchase/premiummembership",{headers:{Authorization:token}});
  // console.log(response.data);
   var option = {
      ...
      const res=await axios.post("http://127.0.0.1:5000/purchase/updatetransactionstatus",{order_id:option.order_id,payment_id:response.razorpay_payment_id },{headers:{Authorization:token}});
        alert("You are premium user now");
        showPremiumMessage(response.razorpay_payment_id);
        console.log();
  ...
  async function download(){
    try{
    const token = localStorage.getItem("token");
    const response= await axios.get("http://127.0.0.1:5000/expense/download", { headers: {"Authorization" : token} })
  ...
  async function showHistory() {
    const token = localStorage.getItem('token')
    const response = await axios.get('http://127.0.0.1:5000/expense/get-history',{ headers: { "Authorization": token } });
  ...
  async function getdata(page){
    ...
    if(isadmin){
      ...
      const limit = document.getElementById('ITEM_PER_PAGE').value|| 5;
      const response=await axios.get(`http://127.0.0.1:5000/expense/getExpenses?page=${page}&limit=${limit}`, { headers: { Authorization: token}})
     console.log(response.data.expenses,"((((((((((((((((9")
      ul.innerHTML="";
      for (let i = 0; i < response.data.expenses.length; i++) {
  ...
  async function updateRows(e) {
    try {
        ...
        const response = await axios.get(`http://127.0.0.1:5000/expense/getExpenses?page=l&imit=${limit}`,{ headers: { "Authorization": token } });
            ul.innerHTML = '';
        for (let i = 0; i < response.data.expenses.length; i++) {
          onScreenFunction(response.data.expenses[i]);
        }
        showPagination(response.data.expenses);
        fetchdata();
    }
    catch (err) {
      console.log(err)
